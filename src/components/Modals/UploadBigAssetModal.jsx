  ...
  const handleFileSelect = useCallback(
    (event) => {
      ...
      if (selectedFiles.length > 1) {
        Notification({ message: 'Please select only one file at a time.', type: 'warning' });
      }
      const validFiles = validateFiles(selectedFiles.slice(0, 1)); // Only allow the first file
      setFiles(validFiles);
  ...
  const handleDrop = useCallback(
    (e) => {
      ...
      if (droppedFiles.length > 1) {
        Notification({ message: 'Please drop only one file at a time.', type: 'warning' });
      }
      const validFiles = validateFiles(droppedFiles.slice(0, 1)); // Only allow the first file
      setFiles(validFiles);
      validFiles.forEach((file) => {
        if (!fileStates[file.name]) {
          startUpload(file);
